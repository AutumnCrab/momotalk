...
def today_key_events_context(persona, now=None, max_items=2):
    ...
    def _is_key(entry):
        # (W)=바쁨(알바·훈련·순찰 등)도 '오늘 있었던 굵직한 일'이다.
        # 예전엔 (O)/(X)만 봐서, 세리카가 저녁 내내 알바를 했는데도 요약에 안 잡혀
        # "오후에 대책위 업무 마치고 쉬는 중"처럼 알바를 통째로 빠뜨리고 답하는 일이 있었다.
...
def build_wake_note(pending_msgs, now=None, reason="sleep"):
    ...
    if not pending_msgs:
        return ""
    if reason == "busy":
        situation = ("선생님이 아래 메시지를 보냈을 때 '너'는 일(회의·알바 등)로 바빠서 못 봤고,"
                     " 이제 막 여유가 생겨 확인한 상황이다.")
        hint = "바빠서 이제야 봤다는 걸(늦게 답해 미안하다는 뉘앙스로) 자연스럽게 티내면서,"
    else:
        situation = ("선생님이 아래 메시지를 보냈을 때 '너'는 자고 있어서 못 봤고,"
                     " 지금 막 깨어나 몰아서 확인한 상황이다.")
        hint = "자느라 이제야 봤다는 걸 자연스럽게 티내면서,"
    lines = [
        ...
        "[중요] 못 보고 있던 쪽은 '너'다. 선생님이 연락을 안 했거나 바빴던 게 아니다."
        " '이제야 연락이 오네', '하루 종일 바빴어?' 처럼 선생님이 늦게 연락한 것으로 몰아가지 않는다"
        " (선생님은 이미 아래 시각에 먼저 톡을 보냈다).",
...
def build_system_prompt(persona, now=None, wake_note="", proactive_note="", event_note=""):
    ...
    lines += [
        ...
        "- [모순을 지적받았을 때] 선생님이 '아까는 ~라더니?', '그거 아니었어?' 처럼 네 말의 모순을"
        " 짚으면, 그 구멍을 메우려고 스케줄에 없는 새 사건을 지어내지 않는다(예: '갑자기 급한 일이"
        " 생겨서', '방금 연락이 와서' 같은 즉석 설정 추가는 금지). 그건 거짓말이 거짓말을 부르는"
        " 전형적인 실수다. 대신 [지금 하는 일]·[오늘 있었던 주요 일정]에 적힌 사실을 다시 확인해서,"
        " 네가 틀렸으면 솔직하게 정정하고(예: '아, 헷갈렸네. 알바는 아까 끝났어'), 선생님이 착각한"
        " 거면 사실대로 알려준다. 주어진 사실만으로 답할 수 있으면 굳이 새 이야기를 만들지 않는다.",
